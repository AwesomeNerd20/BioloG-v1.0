...
public:
	...
	UFUNCTION(Category="UI Achievement", BlueprintNativeEvent)
	...
	virtual void SetValue_Implementation(FAchievementData Achievement, FAchievementStates State, bool ShowExactProgress, UAchievementSystemSettings *Settings);
...
private:
	...
	UPROPERTY(meta = (BindWidgetAnimOptional), Transient)
		UWidgetAnimation* Custom;

	FTimerHandle RemoveTimer;
	FTimerHandle RemoveTimerAnimationComplete;
	
	UFUNCTION(Category = "UI Achievement")
	void OnRemoveStartEvent(UWidgetAnimation *Animation, float AnimationLength);
	UFUNCTION(Category = "UI Achievement")
